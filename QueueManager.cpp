...
void QueueManager::UserQueue::removeDownload(QueueItem* qi, const UserPtr& aUser, const string& aToken) {
...
void QueueManager::UserQueue::removeQI(QueueItem* qi, const UserPtr& aUser, bool removeRunning /*true*/, bool addBad /*false*/, bool removeBundle /*false*/) {
	...
	if (bundle) {
		...
		if (qi->getBundle()->removeUserQueue(qi, aUser, addBad)) {
			...
			if (!removeBundle) {
				getInstance()->fire(QueueManagerListener::BundleSources(), bundle);
			}
		} else {
...
void QueueManager::searchBundle(BundlePtr aBundle, bool newBundle, bool manual) {
	...
	if (newBundle) {
		...
		if (!aBundle->getRecent()) {
			...
			tmp2.resize(snprintf(&tmp2[0], tmp2.size(), CSTRING(NEXT_RECENT_SEARCH_IN), (nextRecentSearch - GET_TICK()) / (60*1000)));
		}
...
void QueueManager::recalculateSearchTimes(BundlePtr aBundle) {
	Lock l (cs);
	if (!aBundle->getRecent()) {
		int prioBundles = 0;
		...
		if(qiFinished) {
...

			{
				...
				if(BOOLSETTING(KEEP_FINISHED_FILES)) {
					...
					fire(QueueManagerListener::Removed(), q);
					fileQueue.remove(q, q->getBundle() ? false : true);
				}
			}
		} else {
			Lock l (cs);
...
void QueueManager::removeQI(QueueItem* q, bool moved /*false*/) noexcept {
	...
	dcassert(q);
	{
		...
		if(q->isRunning()) {
			for_each(q->getDownloads().begin(), q->getDownloads().end(), [&] (Download* d) { x.push_back(&d->getUserConnection()); } );
	...
	for_each(x.begin(), x.end(), [] (UserConnection* u) { u->disconnect(true); } );
}
...
void QueueManager::setQIPriority(QueueItem* q, QueueItem::Priority p, bool isAP /*false*/, bool isBundleChange /*false*/) noexcept {
...

	{
		...
		if((q != NULL) && (q->getPriority() != p) && !q->isFinished() ) {
			...
			if (b->getFileBundle() && !isBundleChange) {
...
void QueueLoader::startTag(const string& name, StringPairList& attribs, bool simple) {
	...
	if(!inDownloads && name == "Downloads") {
		...
		BundlePtr bundle = BundlePtr(new Bundle(bundleTarget, added));
		if (!prio.empty()) {
		...
		if(cur == NULL && name == sDownload) {
			...
			if(size > 0 && start >= 0 && (start + size) <= cur->getSize()) {
				...
				if (cur->getBundle()) 
					cur->getBundle()->addDownloadedSegment(size);
...
void QueueLoader::endTag(const string& name, const string&) {
	...
	if(inDownloads || inBundle || inFile) {
		if(name == "Downloads") {
			...
			if (curBundle->getFileBundle() && !curBundle->getQueueItems().empty()) {
				QueueManager::getInstance()->addBundle(curBundle, true);
			}
			cur = NULL;
		}
...
void QueueManager::getForbiddenPaths(StringList& retBundles, const StringPairList& paths) {
	...
	for_each(hash.begin(), hash.end(), [&](BundlePtr b) { hashBundle(b); });
...
void QueueManager::removeDir(const string aSource, const BundleList& sourceBundles, bool removeFinished) {
	...
	if (sourceBundles.size() == 1 && sourceBundles.front()->getTarget().find(aSource) == string::npos) {
		...
		for (auto i = finishedRemove.begin(); i != finishedRemove.end(); ++i) {
			UploadManager::getInstance()->abortUpload((*i)->getTarget());
			File::deleteFile((*i)->getTarget());
		}

		for_each(ql.begin(), ql.end(), [&] (QueueItem* qi) { removeQI(qi, false); });
	} else {
		for_each(sourceBundles.begin(), sourceBundles.end(), [&] (BundlePtr bundle) { removeBundle(bundle, false, removeFinished); });
	}
}
...
void QueueManager::moveBundle(const string& aSource, const string& aTarget, BundlePtr sourceBundle, bool moveFinished) {
	...
	if (hasMergeBundle) {
		...
		{
			Lock l (cs);
			/* convert the directory names */
			auto dirs =  sourceBundle->getBundleDirs();
			for (auto i = dirs.begin(); i != dirs.end(); ++i) {
				string newDir = convertMovePath(i->first, aSource, aTarget);
				string dir = Util::getDir(i->first, false, false);
				string releaseDir = AirUtil::getReleaseDir(dir);
				int dirItems = i->second;

				if (!releaseDir.empty()) {
					bundleDirs.erase(releaseDir);
				}
				sourceBundle->getBundleDirs().erase(dir);
				sourceBundle->getBundleDirs()[newDir] = dirItems;
				releaseDir = AirUtil::getReleaseDir(newDir);
				if (!releaseDir.empty()) {
					bundleDirs[releaseDir] = dir;
				}
			}
...
void QueueManager::splitBundle(const string& aSource, const string& aTarget, BundlePtr sourceBundle, bool moveFinished) {
	...
	BundlePtr tempBundle = BundlePtr(new Bundle(aTarget, GET_TIME()));
...
void QueueManager::moveFileBundle(BundlePtr aBundle, const string& aTarget, const string& aSource) noexcept {
	...
	if (!move(qi, aSource.empty() ? aTarget : (convertMovePath(qi->getTarget(), aSource, aTarget)))) {
		return;
	}
...
void QueueManager::move(const StringPairList& sourceTargetList) noexcept {
	...
	if (!ql.empty()) {
		...
		if (targetBundle) {
			...
			for_each(newBundles.begin(), newBundles.end(), [&](BundlePtr b) { addBundle(b, false); });
		}
...
void QueueManager::moveBundleItem(QueueItem* qi, BundlePtr targetBundle, bool fireAdded) {
	BundlePtr sourceBundle = qi->getBundle();
	sourceBundle->removeQueue(qi, false);
	userQueue.removeQI(qi, false); //we definately don't want to remove downloads because the QI will stay the same
...
void QueueManager::removeBundleItem(QueueItem* qi, bool finished, bool moved /*false*/) {
	BundlePtr bundle = qi->getBundle();
	if (!bundle) {
		return;
	}
	...
	//LogManager::getInstance()->message("QueueManager::removeBundleItem, token: " + qi->getBundleToken());
	{
		...
		if (bundle) {
			...
			if (finished) {
				...
				for (auto s = bundle->getFinishedNotifications().begin(); s != bundle->getFinishedNotifications().end(); ++s) {
...
void QueueManager::deleteBundle(BundlePtr aBundle) {
	if (!aBundle->getFileBundle()) {
		...
		for_each(aBundle->getFinishedFiles().begin(), aBundle->getFinishedFiles().end(), [&] (QueueItem* qi) { fileQueue.removeTTH(qi); } );
	}
